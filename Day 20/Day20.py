class Grid:
    ...
    def top_left(self): #Start laying out tiles, 
        ...
        t, b, r, l = self.all_bords(tile)
        #rotate tile until the shared borders are on the left and bottom
        ...
        self.tiledict.pop(tile) #remove tile from tile list
        self.bdict.pop(tile)    #remove tile from list of borders
    ...
    def next_grid(self):
        ...
        if "L" in bords and "T" not in bords:
            lb = bords["L"]
            for tile in self.bdict:
                bd = self.bdict[tile]
            ...
            self.tiledict[tile] = tilegrid
            _, b, r, _ = self.all_bords(tile)
        elif "T" in bords and "L" not in bords:
            tb = bords["T"]
            for tile in self.bdict:
                bd = self.bdict[tile]
    ...
    def join_rows(self, poslist):
        ...
        for i, line in enumerate(poslist):
            ...
            for j in range(lenr):
                strlist.append("")
            for square in line:
                for k, string in enumerate(grid.postile[square]):
                    strlist[k] += string
            for rowstr in strlist:
                grid.image.append(rowstr)
